...
//		XSLTStep.transfac = new T
		...
		try {
			...
			XSLTStep.transfac = new MyTransformerFactory() {
				...
				@Override
				public Transformer newTransformer(Source xsl)
						throws TransformerConfigurationException {
					
//					if(useHe){
//						TransformerFactory transfacHe = TransformerFactory.newInstance("net.sf.saxon.TransformerFactoryImpl", null);
//						return transfacHe.newTransformer(xsl);
//					}
					
					
					
					Transformer transformer = pluginWorkspaceAccess.getXMLUtilAccess()
											.createXSLTTransformer(xsl, null, EscaliPluginConfig.config.getSaxonVersionForOxy());
					if(outputResolver != null){
						try {
							...
							if(ReflectionUtil.hasMethod(transformerClass, "getUnderlyingController")){
								Method getUnderlyingControler = transformerClass.getMethod("getUnderlyingController", null);
							...
							if(controler != null){
								Method setOutputURIResolver = controler.getClass().getMethod("setOutputURIResolver", OutputURIResolver.class);
								setOutputURIResolver.invoke(controler, new Object[]{outputResolver});
							}
//							for (Method m : controler.getClass().getMethods()) {
//								if(m.getName().equals("setOutputURIResolver")){
//									m.invoke(controler, new Object[]{outputResolver});
//								}
//							}
...
//							TransformerImpl transImpl = (TransformerImpl) transformer;
