...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    xmlns:sqf="http://www.schematron-quickfix.com/validator/process" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:foo="foo.com">
    <sch:title>Schematron unit test - foreign global</sch:title>
    <sch:ns uri="foo.com" prefix="foo"/>
    <sch:pattern id="pattern1">
        <sch:rule context="foo:foo">
            <sch:assert test="foo:bar/@id[contains(., foo:id(current()))]" sqf:fix="replaceBar">Bar should contain id of foo.</sch:assert>
    ...
    <xsl:key name="foo-id" match="foo:foo" use="@id"/>
    
    <xsl:function name="foo:id">
        <xsl:param name="foo"/>
        <xsl:sequence select="$foo/(@id, concat('id_', count($foo/preceding::foo:foo) + 1))[1]"/>
    </xsl:function>
    <sch:let name="allFooBar" value="//(foo:foo|foo:bar)"/>
    ...
    <xsl:template name="createBar">
        ...
        <xsl:for-each select="1 to $counter">
            <foo:bar barId="{$id}_{.}" generic="{$isGeneric}"/>
        </xsl:for-each>
    </xsl:template>
