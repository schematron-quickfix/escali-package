...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <sch:pattern id="pattern1">
        ...
        <sch:rule context="foo">
            ...
            <sqf:fix id="addBar">
                <sch:let name="target" value="if($mode = 'pattern1') then $rootId else 'bar'"/>
                <sqf:description>
                    <sqf:title>Add bar element with content.</sqf:title>
                </sqf:description>
                <sqf:user-entry name="element" type="xs:string" default="$fooId">
                    <sqf:description>
                        <sqf:title>Enter content for new bar element (Default=<sch:value-of select="$fooId"/>)</sqf:title>
                    </sqf:description>
                </sqf:user-entry>
                <sqf:add node-type="element" target="{$target}" select="$element"/>
            </sqf:fix>
        </sch:rule>
    </sch:pattern>
</sch:schema>
