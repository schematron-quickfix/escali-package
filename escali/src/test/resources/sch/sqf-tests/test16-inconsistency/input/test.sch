<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:es="http://www.escali.schematron-quickfix.com/" queryBinding="xslt2" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" xmlns="http://www.w3.org/1999/xhtml">
    <sch:title>Two different language attributes.</sch:title>
    <sch:pattern>
        <sch:rule context="processing-instruction()">
            <sch:report test="name() = 'pi'" sqf:fix="addLastChild addFirstChild addAttr">PI error</sch:report>
        </sch:rule>
        <sch:rule context="*:p/@lang">
        ...
        <sch:rule context="comment()">
            <sch:report test="true()" sqf:fix="addLastChild addFirstChild addAttr">Comment test</sch:report>
        </sch:rule>
    </sch:pattern>
    <sqf:fixes>
