    ...
    <xsl:template name="topLevelValidatorExtension">
        <xsl:choose>
            <xsl:when test="$sqf:useSQF">
                ...
                <axsl:variable name="sqf:base-uri" select="base-uri()"/>
            </xsl:when>
            <xsl:otherwise>
...
-->
    <xsl:template name="nodeMatching">
        ...
        <xsl:choose>
            <xsl:when test="$sqf:add">
                <axsl:value-of select="string-join(({$nodes})/es:getNodePath(if (. instance of attribute()) then (..) else (.)), ' | ')" separator=" | "/>
            </xsl:when>
            <xsl:otherwise>
                <axsl:value-of select="string-join(({$nodes})/es:getNodePath(.), ' | ')" separator=" | "/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="multiplyDocsMode">
        ...
        <axsl:variable name="sqf:mode-prefix" as="xs:string*">
            <axsl:sequence select="({$nodes})/root()/(if (base-uri() = $sqf:base-uri) then ('#default') else (generate-id()))"/>
        </axsl:variable>
        ...
        <axsl:for-each select="({$nodes})/root()[base-uri() != $sqf:base-uri]">
            <axsl:processing-instruction name="sqf_doc" select="concat(generate-id(), ' # ', base-uri())"/>
        </axsl:for-each>
    </xsl:template>
    <!-- 
